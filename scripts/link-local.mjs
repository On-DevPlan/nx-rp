...
//
// 覆盖式 shim（默认）：
//   用户敲 `nx-rp` 解析到的真实落点（比如 volta\bin\nx-rp.cmd）本来就在 PATH 上，
//   把那里的启动器备份后原地替换成转发器。PATH 一字不动，已开着的终端立即生效；
//   volta / npm / pnpm 管的文本 shim 一视同仁，跟包管理器无关。
//   覆盖唯一的病根风险是「原件丢了」——早先版本覆盖不留备份，unlink 后全局命令
//   静默消失（审查实测事故）。所以现在覆盖前把原内容逐字节存进安装清单（RECEIPT），
//   unlink 时原样写回。那次事故的根源是「没备份」，不是「覆盖」本身。
...
// v1 格式（旧版脚本）：只有 shims[] 路径清单，读到 v1 时走 legacyUnlink 清理。
...
function readReceipt() {
  try {
    ...
    const v1 = r && Array.isArray(r.shims);
...
// 按 PATH 顺序找 nx-rp 的真实落点：第一个含有任何 nx-rp 变体的目录，
// 就是用户敲 `nx-rp` 时实际命中的那个。返回该目录下所有可文本覆盖的启动器变体
...
function resolveTargets() {
  ...
  for (const d of dirs) {
    ...
    const files = textExts.map((e) => join(d, 'nx-rp' + e)).filter(existsSync);
    if (files.length) return { type: 'text', dir: d, files };
    const exe = join(d, 'nx-rp.exe');
...
function shimInstall() {
  // 旧版清单（v1：专用目录 / 无备份覆盖时代）先清理，避免两套 shim 并存
  const prev = readReceipt();
  if (prev && prev.version !== 2) {
    if (yes) {
      console.log('[link:local] 检测到旧版安装痕迹，先按旧清单清理……');
      legacyUnlink(prev);
      console.log('');
    } else {
      console.log('[link:local] 检测到旧版安装痕迹（执行时将先清理再安装）\n');
    }
  }
  ...
  if (!found) {
    if (auto) {
      console.log('[link:local] --auto：PATH 上没有现成的 nx-rp，跳过（不新建、不动 PATH）。');
      return true;
    }
    console.log('[link:local] PATH 上没有现成的 nx-rp 可覆盖。');
  ...
  const prevV2 = (prev && prev.version === 2) ? prev : null;
  ...
  console.log(`[link:local] 验证: nx-rp version   # 应显示 v${pkg.version}（无需重开终端）`);
  console.log('[link:local] 还原: pnpm run unlink:local');
  return true;
}

// 兜底落点（只在 PATH 上没有可覆盖的 nx-rp 时使用）：**绝不写进包管理器自己的目录。**
...
function shimUnlink() {
  ...
  if (receipt.version === 2) {
    ...
    clearReceipt();
    console.log('[link:local] 还原完成。nx-rp 回到安装前的原件（PATH 从未动过）。');
    return true;
  }

  // v1 旧清单
  legacyUnlink(receipt);
  console.log('[link:local] 还原完成（v1 旧式安装）。');
  return true;
}

function legacyUnlink(receipt) {
  ...
  if (receipt.pathEdit) removeFromUserPath(receipt.pathEdit);
  clearReceipt();
  if (receipt.dir && receipt.dir.includes(SHIM_DIR_NAME)) {
...
// --auto 是尽力而为：link 的任何意外（权限、环境诡异）都不该拖垮 build
const ok = unlink ? shimUnlink() : (mode === 'volta' ? voltaInstall() : (auto ? tryQuietly(shimInstall) : shimInstall()));
if (!unlink && !yes) {
...
function tryQuietly(fn) {
  try { return fn(); } catch (e) {
    console.error(`[link:local] --auto 跳过（${e.message?.split('\n')[0] || e}）`);
    return true;
  }
}
